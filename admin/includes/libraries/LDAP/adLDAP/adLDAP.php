...
class adLDAP
{
    ...
    */
    public function group_members($group, $recursive = null)
    {
        ...
        if ($recursive === null) {
            $recursive = $this->_recursive_groups; // Use the default option if they haven't set it
        }
        // Search the directory for the members of a group
        $info = $this->group_info($group, array("member", "cn"));
        $users = $info[0]["member"];
    ...
    */
    protected function adldap_schema($attributes)
    {
        ...
        if ($attributes["expires"]) {
            $mod["accountExpires"][0] = $attributes["expires"]; //unix epoch format?
        }
        ...
        if ($attributes["manager"]) {
            $mod["manager"][0] = $attributes["manager"];  //UNTESTED ***Use DistinguishedName***
        }
