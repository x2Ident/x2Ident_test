...
class pwgen
{
    ...
    public function __construct(
        ...
        self::initializeStaticVars();

        $this->pwgen = 'pwPhonemes';
    ...
    private function pwPhonemes()
    {
        ...
        do {
            ...
            $c = 0;
            $prev = 0;
            $should_be = self::myRand(0, 1) ? self::VOWEL : self::CONSONANT;
            $first = 1;
            ...
            while ($c < $this->pw_length) {
                ...
                if (
                    ($prev & self::VOWEL) && ($flags & self::VOWEL)
                    &&
                ...
                if ($this->pwgen_flags & self::PW_UPPERS) {
                    if (($first || $flags & self::CONSONANT) && (self::myRand(0, 9) < 2)) {
                ...
                if ($this->pwgen_flags & self::PW_DIGITS) {
                    if (!$first && (self::myRand(0, 9) < 3)) {
                        do {
                            $ch = strval(self::myRand(0, 9));
     ...
     */
    public static function myRand($min = 0, $max = 0)
    {
     ...
     */
    private static function initializeStaticVars()
    {
        if (!self::$initialized) {
