...
function rest_delete () {
    if(!@count($GLOBALS['request'])==0){
        ...
        preg_match('/\/apix(\/index.php|)\/(.*)\?apikey=(.*)/',$request_uri,$matches);
        if (count($matches) == 0) {
    ...
    if(apikey_checker($GLOBALS['apikey'])) {
        ...
        if ($GLOBALS['request'][0] == "read") {
			...
			//XIDENT:
                ...
                foreach ($response as $data)
                {
					$rollen_str = $data['fonction_id'];
                }
				$ordner_arr = array();
				$rollen = explode(";", $rollen_str);
				foreach ($rollen as $rolle)
				{
					$response = DB::query("SELECT folder_id FROM ".prefix_table("roles_values")." WHERE role_id='".$rolle."'");
					foreach ($response as $data)
                	{
						$ordner_id = $data['folder_id'];
						if(!array_key_exists($ordner_id,$ordner_arr)) {
							array_push($ordner_arr,$ordner_id);
						}
	                }
				}
				$GLOBALS['request'][2] = implode(";",$ordner_arr);
			...
			else if($GLOBALS['request'][1] == "userpw2") {
				$username = $GLOBALS['request'][2];
				if(strcmp($username,"admin")==0) {
					rest_error('ADMIN_XIDENT');
				}
            ...
            elseif($GLOBALS['request'][1] == "items") {
                ...
                $response = DB::query("select id,label,url,login,pw, pw_iv, id_tree from ".prefix_table("items")." where id IN %ls", $array_items);
                foreach ($response as $data)
                {
                    // prepare output
                    $id = $data['id'];
...
function rest_error ($type,$detail = 'N/A') {
    switch ($type) {
		//XIDENT:
		case 'ADMIN_XIDENT':
            $message = Array('err' => 'User "admin" is not allowed due to security reasons.');
